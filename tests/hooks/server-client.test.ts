...
describe('ServerClient', () => {
  ...
  it('addObservation sends POST /v1/memories with content', async () => {
    ...
    // Against production on 2026-09-20 the narrative payload returned
    // 400 ValidationError "path":["content"] for every MCP observation_add.
  ...
  it('payload builders omit absent fields', () => {
    ...
    // `content: z.string().min(1)`. Measured against production 2026-09-20:
    // the old payload returned 400 ValidationError "path":["content"] on every
    // MCP observation_add, and tests/server/runtime/server-mcp-routes.test.ts
    // reproduced it 4-red against a real Postgres.
