...
//
// A session_summary job used to be fed ONLY the events the per-event lane had
// not yet collapsed. The per-event lane normally wins that race, so the summary
// job arrived at an EMPTY event list, the prompt then told the model
// "<!-- empty after privacy stripping -->", and the model did exactly what the
// instruction beside it asks for in that case: it returned <skip_summary />.
// The job completed, wrote 0 observations, and the only trace was a flag called
// `privateContentDetected` — nothing had been private, and nothing was wrong
// with the provider.
//
// MEASURED on the patrykradek corpus 2026-09-19: 441 of 738 completed
// session_summary jobs (60%) wrote zero observations that way, including the 8
// whose sessions hold 60-351 events. Replayed live against the same model, the
// empty prompt returns `<skip_summary />` in 5 completion tokens while the same
// session's real events return a full <summary> block.
//
...
// empty input is a NAMED outcome instead of an anonymous `completed`.
